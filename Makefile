...
# FkRecipes arrives through the REAL Go module channel since 0.3.3 (its go/v0.1.1
# tag, no replace), from a public repository: the proxy and the checksum database
# serve it and go.sum holds its lines, so nothing here sets GOPRIVATE. It did for
# the hours the repository was private before 0.3.3 shipped; the export and its
# fetch instructions went with the publication.
