...
The first belt of such a line is the exception, and it is the one to know about before you lay one. Nothing is behind it yet, so on a free face it is an output from the moment it lands, and it stays one until the belt behind it goes down. Lay a line meant to pass the balancer from its source, or start it a tile further out than the balancer, and no belt of it is ever taken; put a belt behind one that was taken, and the balancer hands the port back on the next tick.
...
A balancer can also be too big to give a belt priority, because a priority port takes more room inside the machine than an even one. The limit is 32 belts on the balancer's busiest side. Above it nothing moves: the flag is not set, the balancer keeps running as it was, and a message says so with the number of belts that side carries. Building the balancer smaller than that, or splitting it in two, is the way out.
...
**On Factorio 2.1 most of those balancers will not run again until you rebuild them, and that is worth knowing before you make the cutover save.** This mod allows one belt per balancer part, because 2.1 allows one belt-connectable per tile and every edge of a balancer is one. Belt Balancer's own layout is a single column of parts with a belt on both sides of each, which is two belts per part, so a balancer built that way is converted and then refused: you keep the parts, you keep the items on your belts, and each affected force gets a chat message naming how many balancers need rebuilding, with a clickable map ping per balancer over as many chat lines as the list takes. **The exception is a balancer you already built one belt per part** -- a two-column block, inputs down one side and outputs down the other -- which converts into a working balancer and runs at full rate straight away.
