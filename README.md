...
- The art is a computed placeholder. Every pixel of the 47-variant sprite sheet
  comes from `tools/make-graphics.py` (a signed distance field and a palette).
  It reads as one continuous machine across any shape; an artist replacing the
  PNGs would be an improvement, and the cell order is the only contract.
...
Released under the [MIT License](LICENSE). The computed sprite sheet under
`mod-data/` is part of the same work and carries the same licence. FkLua, which
compiles the guest, is MIT licensed as well.
