...
-- WHETHER THE FORCE CAN SEE THE GROUND THE PINGED BALANCERS STAND ON, AND THE
-- WALL THAT MAKES THAT UNANSWERABLE HERE.
--
...
-- assumed: with `#game.players == 0`, `force.chart` charts nothing,
-- `force.chart_all` over a fully generated surface charts nothing, a powered
-- radar charts nothing, and NAUVIS'S OWN ORIGIN CHUNK -- which every real game
-- charts at world creation -- reads uncharted too. A force with no players has
-- no chart to write into. That puts the EFFECT behind the same player wall as
-- the flying text and the hand-back; what is on this side of it is the guest's
-- own `charted N from x,y to x,y`, which the assertion script reads.
--
...
local function chart_state(tag)
  local s, force = surf(), game.forces.player
  local chunks, charted, n = {}, 0, 0
  for _, y0 in ipairs { ME1, ME2 } do
    for dy = 0, 1 do
      local cx, cy = math.floor(0 / 32), math.floor((y0 + dy) / 32)
      local k = cx .. "," .. cy
  ...
  log(string.format(
    "[FLIP] chart tag=%s force=%d charted=%d of=%d nauvis_origin=%s players=%d",
    tag, force.index, charted, n,
...
local SCHEDULE = {
  ...
  -- has to be refused.
  [60]   = function()
    report_setting("default"); chart_state("default"); audit_now("default")
  end,
  ...
  [1410] = function()
    report_setting("post-veto"); chart_state("post-veto"); audit_now("post-veto")
  end,
  ...
  [3120] = function()
    report_setting("final"); chart_state("final"); audit_now("final")
  end,
}
