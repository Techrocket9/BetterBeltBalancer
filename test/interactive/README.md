...
**A belt over a part.** Hold a south-facing express belt over (20, 98), the middle part of the column. The cursor must show the fast-replace preview rather than a red block. Place it. Expected: the part vanishes, the belt takes the tile, the part arrives in your inventory rather than on the ground, and the column becomes two balancers with the new belt an output of the upper one and an input of the lower one. Run `/bbb-audit`: `drift=0 unbuilt=0`, one more cluster and one fewer part than before. A tile still counted as a part with your belt standing on it is the defect this gesture exists to catch.

**A belt over a part the balancer is using.** Hold the same belt over (20, 96), the top of the column, which carries the balancer's input. Every part is replaceable, including the ones with a belt against them: each of those also holds a hidden belt piece of the mod's own, and both are replaced together. Expected: the part and the hidden piece both go, the belt takes the tile and feeds the part below it, the part arrives in your inventory, and the balancer keeps running one part shorter, because the belt now feeds it where the part used to.

**And the refusal that follows one.** Hold the belt over (20, 97) or (20, 99) instead, the parts either side of the middle. Each of those sits next to a part that already has its one belt, so the belt you lay becomes that part's second and, on Factorio 2.1, the balancer it belongs to cannot be built. Expected: the belt is placed and stays, the balancer stops, and `/bbb-audit` reports `refused=1` with one fewer network than clusters. What the machine was holding is on the ground beside it, because the part went before anything could ask whether what was left could still be built. Mining the belt clears the refusal; putting a part back on the tile is what makes the column a balancer again.
