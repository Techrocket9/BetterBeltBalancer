...
make datastage-check
              ...
              # suite can ask. TWENTY ARMS SINCE FIX ROUND 2,
              # two mod sets HASHED against goldens, sixteen
              ...
              # no mod set this machine can install has one -- and one MERGE
              # arm behind a Lua fixture that DELETES `transport-belt` ahead of
              # this mod, for the same reason turned around. See Verification
...
**`make datastage-check` IS A SECOND GATE AND A DIFFERENT QUESTION, and it is not part of `make test`.** All fourteen suites are about the RUNTIME: they build a world, run it and read the guest's own log lines, and by the time any of them is looking the data stage has been over for a whole load. Not one of them can see a prototype FIELD. `test/check-datastage.py` runs Factorio's own `--dump-data`, which executes the settings and data stages and **stops before `control.lua`**, normalises the result with `jq -S` and hashes it against a golden in `test/datastage-goldens.json`. About three seconds per arm, against the suites' minutes. **TWENTY ARMS SINCE FIX ROUND 2** (nineteen from the re-adoption leg until then, at 41 to 47 seconds, `/usr/bin/time -p make datastage-check`: real 41.30 and 46.96 over two runs on 2026-09-13, the spread being the machine's and not the gate's; eighteen and real 37.32 from fix round 1 until that): the two hashed mod sets, sixteen VARIANT arms, one SPEED arm and one MERGE arm, none of the eighteen hashed and all described below; the EIGHT customizer arms are in "Round three" under "Cost, research and belt speed", with what fix round 2 made of them in "Fix round 2". **THE ARM COUNT IS READ OFF THE SCRIPT AND NOT COUNTED BY EYE**: `check_variants` prints `len(RECIPE_VARIANTS) + len(TECH_VARIANTS) + len(RECIPE_TEXT_ARMS) + len(TECH_COST_ARMS) + len(TECH_TIER_ARMS) + 1`, which is 5 + 2 + 4 + 1 + 3 + 1.
