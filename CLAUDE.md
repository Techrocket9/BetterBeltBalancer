...
**One defect this pass found and did not fix**, because it is a guest decision rather than a test one: a converted-and-refused balancer is announced with the ORDINARY per-piece message -- the one that says the extra piece was left in place unconnected, when nobody placed anything -- and the migration summary with its GPS checklist arrives only in the `added` leg, where a rebuild-from-world happens to follow the conversion. Both arms are pinned in `EXPECT_SUMMARY` so the fix cannot land silently; the measurement, the mechanism and the proposed shape are in [`agents/single-edge.md`](agents/single-edge.md)'s phase-7 section.
