...
**And verified 2026-09-07 with BOTH 0.3.3 FEATURES IN ONE LINE**, which is the state a release is cut from: the belt over any part, the curved exit, and the alert fix the first found and the second's file owned ("The alert that said the machine was fine"). The two branches rebase into one another cleanly and **`fklua.toml`, `mod-data/changelog.txt` and CLAUDE.md are the only files that conflict** -- one version token, one changelog section holding both features, and the Status rows, the dated verification paragraphs and the owed list, which is prose in one file and not a semantic collision anywhere. Nothing in `guest/go` conflicted at all: the data stage is one branch's and `compile.go` the other's, and the one file both bounds' refusal reaches through is `sedge.go`, which only the curve branch had touched.

**THE WHOLE VERIFICATION IS THE `release/2.0` ARM'S AGAIN**, for the third pass running and the same reason: the installed binary is **Factorio 2.0.77 (build 84539)**, confirmed from `--version`, and trunk's `make test` refuses at its engine gate with the packaged mod pinned 2.1. The four commits were cherry-picked onto a worktree branched from `verify/belt-over-part` -- itself `release/2.0`'s 0.2.2 release commit plus the belt-over-part change at 0.2.3 -- so both features land in ONE 0.2.3 section there, exactly as they land in one 0.3.3 section on trunk. **All fourteen suites green in BOTH arms** (3m33s collected, 3m00s leaking, one invocation each), with `flip` running and `mig21`/`mig` inverted, and **the eleven-arm `make datastage-check` green in 31 s** against the 2.0.77 goldens the belt-over-part commit re-captured: `base data_raw_sha256 058da3c95203e4dc`, `incumbent f67c4bf8544a9010`, both `mod_settings_sha256 196275f867f7f8b5`, all eight variant arms and the speed arm. The curved exit is a runtime rule and emits no prototype, so a green golden over both features is the statement that it did not.

**No number either branch recorded moved.** The leaking arm's slopes came back **1,280 / 352 / 1,209 / 32 / 560 / 3,736 / 2,080 B** per primitive over **3.92 MiB** of linear memory, with the new **leg H at 384 B/iter and 192 B/primitive**, every leg's linearity inside its ceiling (x1.00 except C at x1.03 and G at x1.07), 1,136 B of calibration at **0.0% spread** and **0 items lost over 200 teardowns**. `m2`'s curve band is **1306 1304, 1.998x one belt at 0.15% spread**, both lanes occupied on all five samples of both exits (65/50 and 60/60), and `sload` at **exactly 0**. The `edge` suite's fast-replace band is **frepc 264 -> 262** and **frepd 264 -> 0** over equal windows, **one** one-belt-per-part refusal in the whole run and it is inside `frepd`'s window, **6 items spilled** beside that cluster, and 180-197 visible entities of ours with **0 off a part tile** at every sample.

**AND ALL THREE ARMS OF THE FIXED ALERT ARE OBSERVED IN ONE `edge` RUN**, which is what makes the fix a measurement rather than a refactor. `lim`, whose 64-port network is standing: *cluster 67 would need 128 ports for 65 inputs and 1 outputs, over the limit of 64; refused BEFORE the teardown, so the standing network is untouched* -- byte for byte the line that shipped. `brdg`, the merge the pre-pass spared: *cluster 36 ... refused, and this cluster had no network to lose*, with `standing networks the refusal left alone: [2]` on its own line beside it, which is where that reassurance belongs and where it can be checked. And `frepd`: *cluster 49 has 1 part carrying more than one belt, worst 2; ... so the compile is refused AFTER the network on these tiles came down, so its contents are handed back*, with `spilled 6 items beside cluster 49` in the very next line of the log.

Package built from clean on trunk, shipped config: zip **670,844 B**, `fk_module.lua` **3,232,716 B**, `fk_api_gen.lua` **24,283 B**, `fk_data_module.lua` **3,129,344 B**, `dist/bbb.wasm` **1,325,064 B**, members / events / defines **57 / 24 / 4**. Of that the alert fix alone is **+2,813 B of zip (+0.42%)**, +39,971 B of `fk_module.lua` and +6,161 B of wasm, measured against a clean build of the two feature commits without it. The 2.0 arm's package is zip **590,526 B**, `fk_module.lua` **3,233,192 B**, `fk_api_gen.lua` **24,196 B**, same **57 / 24 / 4** against that pin's smaller tables. **`make check` is green on trunk**; on the 2.0 branch every leg passes except `fklua gen-bindings --check`, which is `release/2.0`'s own committed bindings drifting from the FkLua sibling and is inherited rather than caused -- `guest/go/fkapi/fkapi.go` is byte-identical to the 0.2.2 release commit's across all four cherry-picks, and regenerating it is a release decision for that branch.

**WHAT IS OWED IS ITEM 0b'S LIST AND NOTHING WAS ADDED TO IT**: the 2.1 datastage goldens (the 2.1.16 and 2.1.17 rows are 0.3.2's and stale for the one prototype field), `make test` and `make GC=leaking test` on trunk with a 2.1 binary, the `release/2.0` recut at 0.2.3 to ship both features there, that branch's stale bindings, and the interactive gestures -- which gained nothing new here either, both features' own gestures being on the checklist already. The 2.0 verification branch is kept as **`verify/0.3.3-2.0`** for the recut to start from.
...
0b. **THE 2.1 DATASTAGE GOLDENS AND THE TRUNK SUITE RUNS, both owed by 0.3.3 and both blocked on the binary.** The machine was on Factorio 2.0.77 when 0.3.3 was taken, so its whole verification is the `release/2.0` arm's. The 2.0.77 golden rows are re-captured and green; the **2.1.16 and 2.1.17 rows are 0.3.2's and are STALE** -- the same one prototype field moves `data_raw_sha256` on that engine too -- and `test/datastage-goldens.json` says so in its own `_note`. `make datastage-check` SKIPS a golden whose engine does not match the binary, so nothing here is silently green; what is required before 0.3.3 is cut is one `make mod && test/check-datastage.py --capture` on a 2.1 binary and one `make test` plus `make GC=leaking test` on trunk. Neither is expected to move anything but the two hashes: nothing outside the data stage changed, the control guest's emitted Lua is byte-identical, and the eleven-arm gate and all fourteen suites are green on 2.0.77. **AND THE SAME IS OWED FOR THE CURVED EXIT**, which is 0.3.3's other half: it is gated on the `release/2.0` arm in both `-gc` arms and nothing but the goldens is expected to move there either, but `mig21` and `mig` INVERT on 2.1 and their being green on 2.0 says nothing about the other side. The `release/2.0` arm needs a recut (0.2.3) to ship either feature there, and **`make check` on that branch fails at `gen-bindings --check` before any change of this pass** -- its committed 2.0.77 bindings drifting from the FkLua sibling, confirmed on the pristine branch and left alone, because regenerating them is a release decision for that branch.
