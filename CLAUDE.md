...
test/                    headless verification, FIFTEEN suites (see below).
...
**The long form, every measurement and every red proof, is [`agents/fkrecipes-migration.md`](agents/fkrecipes-migration.md), "Fix round 2b re-adoption".** FkRecipes `221ff9c` to `61ac80c`, three commits, and the block this mod's previous round ended on lifts on the middle one.
...
make guest    # TinyGo -> dist/bbb.wasm.  QUIET=1 drops the [BBB] log lines
              ...
              # stays green on all fifteen suites. See "The third decision"
              #
...
make test     # headless verification, FIFTEEN suites, and the DEFAULT is all
...
make datastage-check
              ...
              # suite can ask. TWENTY-THREE ARMS SINCE FIX ROUND 2'S
              # CEILING COMMIT, two mod sets HASHED against goldens, sixteen
              ...
              # no mod set this machine can install has one -- and TWO MERGE
...
**`make datastage-check` IS A SECOND GATE AND A DIFFERENT QUESTION, and it is not part of `make test`.** All fifteen suites are about the RUNTIME: they build a world, run it and read the guest's own log lines, and by the time any of them is looking the data stage has been over for a whole load. Not one of them can see a prototype FIELD. `test/check-datastage.py` runs Factorio's own `--dump-data`, which executes the settings and data stages and **stops before `control.lua`**, normalises the result with `jq -S` and hashes it against a golden in `test/datastage-goldens.json`. About three seconds per arm, against the suites' minutes. **TWENTY-ONE ARMS SINCE FIX ROUND 2'S REMOVER COMMIT** (twenty from that round's adopt commit until then; nineteen from the re-adoption leg before that, at 41 to 47 seconds, `/usr/bin/time -p make datastage-check`: real 41.30 and 46.96 over two runs on 2026-09-13, the spread being the machine's and not the gate's; eighteen and real 37.32 from fix round 1 until that. **THE WALL TIME AT TWENTY-ONE ARMS IS NOT RE-TAKEN** and is owed to that round's close): the two hashed mod sets, sixteen VARIANT arms, one SPEED arm and TWO MERGE arms, one per mod set, none of the nineteen hashed and all described below; the EIGHT customizer arms are in "Round three" under "Cost, research and belt speed", with what fix round 2 made of them in "Fix round 2". **THE ARM COUNT IS READ OFF THE SCRIPT AND NOT COUNTED BY EYE**: `check_variants` prints `len(RECIPE_VARIANTS) + len(TECH_VARIANTS) + len(RECIPE_TEXT_ARMS) + len(TECH_COST_ARMS) + len(TECH_TIER_ARMS) + 1`, which is 5 + 2 + 4 + 1 + 3 + 1, and `check_remover` prints `len(ARMS)`, which is the same 2 the golden arms are, so the total is `2 + 16 + 1 + 2`.
...
What a human has to see: the fast-replace preview rather than a red block over a belt; the belt and its cargo arriving in the INVENTORY rather than on the ground (a script build has no player, so every headless number here is the spill arm); the reverse gesture handing the PART back the same way, on an edge part as much as an interior one; and the DRAG taking out every part it crosses. Grep for `a belt-connectable fast-replaced the part at`, for `compiled cluster … 3->2` and for `carrying more than one belt`.
...
**The 65-input cluster is refused, once, and the world is untouched.** Exactly one `[BBB] alert: cluster … would need 128 ports for 65 inputs and 1 outputs, over the limit of 64; refused` in every bbb create, and **zero `[BBB] error:` in any log of the whole session** — which is the other half of the assertion, because the failure mode this replaced was three errors and a demolished network per belt ([`agents/maxports.md`](agents/maxports.md) §4). `run.sh` fails a mega cell that does not carry the alert.
