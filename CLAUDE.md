...
guest/go/obs/            THE TEST OBSERVERS, and the second half of "no
                         ...
                         test/assert-*.py. THE ONE OBSERVER THAT IS NOT HERE IS
                         mig21: it was ported a second time, to RUST, in phase 8
                         and lives at guest/rust/obs/mig21 below. They are thin
                         `main`s
...
guest/rust/              THE ESTATE'S ONE RUST OBSERVER, and the reason the
                         estate is mixed-language. FkLua generates Go and Rust
                         bindings at MEMBER-ID PARITY and guards that with
                         mirror tests against a host STUB; obs/mig21 holds the
                         same claim against a REAL ENGINE and two committed
                         fixtures, on every `make test`, forever. It is the
                         program guest/go/obs/mig21 was, and it produces the
                         same 51 log lines on the same two saves BYTE FOR BYTE
                         (agents/estate-port.md, phase 8). mig21 is the suite
                         that makes that a fair test rather than a lucky one: no
                         storage, no world-building, no rigs -- it reads and it
                         logs -- so a transcript difference is a difference in
                         the BINDINGS, the marshalling or the arithmetic and
                         cannot be a difference in a rig. It is also the first
                         observer with no fk_on_init at all: it builds no world,
                         ...
                         `guest/rust` BESIDE `guest/go` IS THE TOOL'S CHOICE AND
                         NOT TASTE: `fklua gen-bindings` writes Rust bindings to
                         exactly guest/rust/fkapi the way it writes Go's to
                         exactly guest/go/fkapi, and `fklua lock` hashes both at
                         those names -- a project anywhere else ORPHANS the
                         generated crate. There is no second fklua.toml either:
                         the mod's own manifest carries lang = ["go", "rust"],
                         so it is ONE MANIFEST, ONE PIN, TWO OUTPUTS. A
                         directory-scoped project would have been a SECOND PIN,
                         and two pins is how an observer ends up calling
                         different member ids from the mod it observes -- ids
                         are dense sorted indices over one description's set, so
                         the two would not even fail loudly. It is a cargo
                         workspace; obs/mig21 carries its own line builder and
                         its own three-function slice of the harness, because
                         one consumer is no duplication to remove
                         (`_initialize` rather than a package initialiser is the
                         one export that differs from the Go original, and it is
                         forced: Rust has no pre-main initialiser in a cdylib
                         reactor, and fk_on_init would never fire in a suite
                         with no --create phase)
...
make observers
              ...
              #
              # THIRTEEN ARE TINYGO AND ONE IS CARGO. mig21's observer was
              # re-ported to RUST in phase 8 (guest/rust/obs/mig21), so `make
              # test` needs rustc 1.97.1 with the wasm32-unknown-unknown target
              # as well as TinyGo. It is ~11 s cold and a no-op warm, and
              # `rustup target add wasm32-unknown-unknown` is the one new
              # install; wasm-opt was already a hard dependency of the TinyGo
              # build, and the Rust recipe's extra --llvm-memory-copy-fill-
              # lowering pass is a step rather than a dependency. See the
              # Makefile's own block for why that pass and the RUSTFLAGS are
              # BOTH necessary -- they do different halves of one job
```
...
**And verified 2026-08-25 after PHASE 8, WHICH CLOSED THE ON-ENGINE PROGRAMME** -- `mig21`'s observer written a SECOND time, in RUST ([`agents/estate-port.md`](agents/estate-port.md)). The estate is thirteen Go observers and one Rust one, and `guest/go/obs/mig21` is deleted. **The gate is CROSS-LANGUAGE TRANSCRIPT IDENTITY and it is met exactly: 51 tagged lines over both committed fixtures, byte-identical, with NO MASK AT ALL** -- proved by self-diff first, two independent runs of the unmodified tree agreeing from the `[MIG21]` tag onwards, so the only thing dropped is Factorio's own timestamp and the `control.lua:NNN` the generated Lua logs from, neither of which is the observer's output. Three more parity numbers came with it and none was assumed: the packaged `fk_api_gen.lua` is **BYTE-IDENTICAL across the two languages** (20 members pruned from 4859, the same 1 event and the same 1 define -- two backends' pruning passes over two entirely different wasm modules selecting the same members and assigning the same dense ids), `fklua api check --from 2.1.16 --to 2.0.77` is **clean with 0 findings on both** over an identical surface (20 members, 1 event, 5 concepts) so the Rust observer stays STAMPED, and the Rust `fk_module.lua` is **493,791 B against Go's 577,418 (-14.5%)** for the same program. **All fourteen suites green in BOTH arms** (2m23.7s collected, 2m26.7s leaking, one invocation each) and **no suite's number moved at all** -- the `mar` slopes back **identical to the byte**, 1,280 / 352 / 1,209 / 32 / 560 / 3,736 / 2,080 B per primitive over **3.92 MiB**, 1,136 B of calibration at 0.0% spread, collected at 9 collections in 6 paced steps with 0 forward-progress deadlines. **Cargo is a `make test` dependency now** (rustc 1.97.1, `wasm32-unknown-unknown`): ~11 s cold, a no-op warm.
