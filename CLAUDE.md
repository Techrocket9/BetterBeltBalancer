...
**THREE BEHAVIOURS MOVED WITH `CostBy` AND ALL THREE ARE ON THE RECORD**: the fallback's science pack is probed even when the fallback is unreachable (a load that used to succeed and now refuses, AWKWARD), `max_level` travels with the copied unit (a single-recipe technology becoming multi-level in a pack whose source is, AWKWARD), and a `count_formula` source that used to abort the load now loads (an improvement). Each is measured on the engine and pinned by a host test; "The technology, where the prerequisite moves with the unit" is the long form.

**AND THE UNKNOWN-OPTION ARM OF `RecipePlan` AND `TechLadder` IS A SHAPE GUARD RATHER THAN A LIVE FALLBACK.** [Plan] builds one choice per allowed value, so an unknown stored value reaches the library and comes out as a recipe with no ingredients and a fallback-priced technology, not as this package's default. What makes that unreachable is the ENGINE: an unknown value in `mod-settings.dat` is silently RESET to the default before the data stage runs (measured on both packages, no log line), and a wrong TYPE is refused loudly. Both comments say so now, the test that asserted the dead arm is deleted, and `TestAnUnknownOptionIsWhatTheLibraryDoesToday` records the other side of the engine's guard.
