...
**Sizes at this commit against round three's, same machine, shipped config**: the zip 822,248 to **817,198 B**; `fk_data_module.lua` 4,904,124 B and 122,031 lines to **4,470,110 B and 112,279 lines**; `fk_module.lua` 3,148,568 to 3,132,893 B; `dist/bbbdata.wasm` 1,085,173 to 1,096,842 B; relay stations 13 to 12. Of that, the TOOLCHAIN's share is measured on round three's exact wasm files repackaged with the new fklua before anything was relinked: 4,318,883 B and 108,919 lines and 11 stations, a tenth off the data module; the library's eight commits put a quarter of it back. The table with every command is in the migration note, and the numbers move again in this round's later commits, and the round's closing table is the one that ships.
