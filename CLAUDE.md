...
guest/go/tune/           WHAT THE DATA STAGE DECIDES that is not fixed: the
                         ...
                         Custom fields as GENERATED names placed under their
                         dropdowns by `OrderAfter`, because nothing has shipped
                         them. SINCE ROUND TWO IT DECLARES THE
...
| `custom` (since 0.3.3) | whatever `better-belt-balancer-recipe-ingredients` says, in FkRecipes' ingredient language; the word `default` there is the vanilla plan WITH its ladders. "Round three" below |
...
`bbb-tech-cost`, the same shape: `logistics` (DEFAULT, today's behaviour), `logistics-2`, `logistics-3`. The strings ARE the base technology names, so there is no mapping table under them. Since 0.3.3 a fourth value, `custom`, prices the research from `better-belt-balancer-tech-packs`, `better-belt-balancer-tech-count` and `better-belt-balancer-tech-seconds` (generated names placed by `OrderAfter` since the sync pass) and places it by a ladder of its own, which since fix round 1 is the tier list itself; "Round three" below.
...
**THE CUSTOM ARM KEEPS THIS RULE SINCE FIX ROUND 1, BECAUSE ITS LADDER IS THE TIER LIST ITSELF.** A written cost names no source, so its place comes from a ladder of its own, and that ladder is `TechOptions()`, cheapest first, so its head is `TechDefault()` BY CONSTRUCTION rather than by a second transcription that can drift. The placeholder in the three fields is [FallbackUnit], base's own `logistics` unit, so Custom with nothing edited is 20 red science sitting after Logistics, which is exactly where the default tier already put it. **Measured on 2.0.77 through the gate's own `run_arm`: that configuration's normalised data-raw dump hashes `1e1fcf4f56f5ef22`, byte-identical to the whole default dump**, so not one prototype in the table moves and 0.3.3's own promise that picking Custom changes nothing is true of the tree as well as of the price. **THAT MEASUREMENT'S SCOPE IS A MOD SET THAT HAS `logistics`, and it is stated rather than left to be read off the dump's mod list.** Take the head away and the two dropdown readings PART, because a tier's ladder is `TechLadder` and the default tier's is that one name where this one has two rungs under it: measured on the host in a game holding `logistics-2` and `logistics-3` and nothing below them (`cd guest/go && go test ./tune/ -run TestTheCustomArmTakesItsPlaceFromTheDefaultTier`), the DEFAULT tier emits no `prerequisites` field at all and says so (`no source for the logistics cost carries a unit, so the fallback cost applies and the technology has no prerequisite`), while Custom untouched emits `["logistics-2"]`, both on the same [FallbackUnit]. Picking Custom THERE does move the research, from nowhere to Logistics 2, which is the nearest legal place and is what the rungs under the head are for. So the changelog entry, the `bbb-tech-cost` locale entry and README's research paragraph each say "does not move" of a game that HAS Logistics and state the other case as its own clause. Until fix round 1 the ladder started at `logistics-3`, the same configuration hashed `afb19d7fa0cfd0a5` with `prerequisites` the one differing field, and that put 20 red science behind base's own gate on that technology, `["production-science-pack", "lubricant"]`: the mismatch this section argues against, with the sign flipped, for as long as the player had not typed. The argument for it reasoned about the numbers a player is ABOUT TO write, and a ladder decides the state they are IN. `TestTheCustomResearchCostUntouchedIsTheFallbackUnit`, `TestTheCustomArmTakesItsPlaceFromTheDefaultTier` (which asserts the head against `TechDefault()` rather than against the literal, so the COUPLING is what fails when somebody pulls the two ends apart, and carries the without-`logistics` pair above) and the gate's `tech-custom-default` arm pin the placement, and **the flip took that arm's anti-vacuity away, which was replaced rather than noted**: both readings are now `["logistics"]` with the same unit, so the library's `takes its research cost from` line is the only thing separating a `mod-settings.dat` that arrived from one that never did, and the arm asserts it FIRST, which is `check_speed`'s discipline.
...
**THE GATE IS NINETEEN ARMS SINCE THE RE-ADOPTION LEG**: the two hashed mod sets, fifteen variant arms, the speed arm and the merge arm (eighteen and fourteen from fix round 1 until then). The seven customizer arms drive the real settings through the `.dat`: `recipe-custom-default`, `recipe-custom`, `recipe-ignored`, `recipe-self-product`, `tech-custom-default`, `tech-custom`, `tech-ignored`; six of them assert the library's own log lines (`tech-ignored` the whole three-line stream, in order, since the sync pass; `recipe-self-product` its two, in order, since the re-adoption leg), which are the only thing in that gate not read out of a dump, because what the library does with a text under a preset, or with a list that names the product, is by design invisible in the prototypes. `recipe-self-product` writes the mod's own part into the ingredient text, which base 2.0.77's own `kovarex-enrichment-process` is the precedent for the engine accepting; like the merge arm it carries a load-completes claim, and the mechanism is every arm's, `run_arm` exiting the process on a non-zero engine return, so an arm reaching its assertions at all is the exit code read. `tech-custom` is also where the engine reading the stored count is confirmed rather than assumed (a reset count would read back 20 and fail the unit by that field; round three wrote it as a type-2 double and the sync pass writes it as the signed 64-bit, which is what the engine itself stores an int setting as), and `tech-ignored` writes all three fields under `logistics-2` and asserts logistics-2's own unit, so a number of this mod's reaching the unit under a tier fails by that field.
...
make datastage-check
              ...
              # suite can ask. NINETEEN ARMS SINCE THE RE-ADOPTION LEG,
              # 41 to 47 s: two mod sets HASHED against goldens, fifteen
              # VARIANT arms (one per
              # non-default value of each cost setting plus the seven
...
**`make datastage-check` IS A SECOND GATE AND A DIFFERENT QUESTION, and it is not part of `make test`.** All fourteen suites are about the RUNTIME: they build a world, run it and read the guest's own log lines, and by the time any of them is looking the data stage has been over for a whole load. Not one of them can see a prototype FIELD. `test/check-datastage.py` runs Factorio's own `--dump-data`, which executes the settings and data stages and **stops before `control.lua`**, normalises the result with `jq -S` and hashes it against a golden in `test/datastage-goldens.json`. About three seconds per arm, against the suites' minutes. **NINETEEN ARMS SINCE THE RE-ADOPTION LEG and 41 to 47 seconds (`/usr/bin/time -p make datastage-check`: real 41.30 and 46.96 over two runs on 2026-09-13, the spread being the machine's and not the gate's; it was eighteen arms and real 37.32 from fix round 1 until then)**: the two hashed mod sets, fifteen VARIANT arms, one SPEED arm and one MERGE arm, none of the seventeen hashed and all described below; the seven customizer arms are in "Round three" and "Re-adoption leg" under "Cost, research and belt speed".
...
**A VARIANT ARM DRIVES THE REAL SETTING, through a `mod-settings.dat` the gate WRITES.** That is the only way to ask a settings stage a question from outside, there is no flag for it, and there is no Lua in this repository to do it with. The format is Factorio's own binary property tree, and THE WRITER IS THE TOOLCHAIN'S since the sync pass: `fklua modsettings write` (FkLua c21ff07 or later), which replaced the transcription this repository carried and had verified by round-tripping the engine's own file; the gate probes the binary before the engine is asked anything and refuses NOT RUN with the remedy when it lacks the command. A number's JSON spelling is its type, an int setting written `50` and a double `20.0`, which is the engine's own typing: it writes an int setting back as the signed 64-bit, read out of its own rewrite. **Its anti-vacuity is structural for ELEVEN of the fourteen**: if the file were ignored or unreadable the arm would read back the DEFAULT recipe or the default tier's cost, and the assertion is an equality against something else. **THREE ARMS EXPECT EXACTLY WHAT A MISSING FILE PRODUCES, and each is accounted for by name rather than left to be found.** `recipe-vanilla` is one on purpose: its claim IS that a written default and no file at all agree, which is a statement about the writer. `recipe-custom-default` is coverage of a documented path, and its own comment says, measured, that it carries none of that weight. `tech-custom-default` was separated by its PREREQUISITE until fix round 1 flipped the `Position` ladder; both readings are now `["logistics"]` with the same unit, so it demands the library's `fkrecipes: bbb-balancer takes its research cost from ...` line FIRST, before it believes either claim, because that line is written only under `custom`. Red-proven by running that arm with no file at all, and controlled by the same run with the assertion deleted, which reports `ok` and exits 0.
