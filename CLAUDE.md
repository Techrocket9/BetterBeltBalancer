...
> **TRUNK TARGETS FACTORIO 2.1 SINCE 2026-08-24, AND ITS RULE IS ONE BELT PER BALANCER PART.** 2.1 closed the collision-mask loophole that let two interface linked belts share a part's tile, so a part carries at most one edge and a cluster that asks for more is refused. The design is [`agents/single-edge.md`](agents/single-edge.md) and the rule is `guest/go/sedge.go`; "One belt per balancer part" below is the working note. **Nine of the eleven suites do not run on 2.1** -- every rig in them is built in the multi-edge idiom -- so every number in this file below that line was measured on 2.0.77 and is kept as measured until the estate is rebuilt. `m1` and `sedge` are what `test/run.sh` runs today.
...
guest/go/carry/          the carry-pool IDENTITY: `Region` (surface, force,
                         ...
                         the tiles that were already there. Read its header
                         before touching either, and agents/single-edge.md
                         before touching anything about the port
...
mod-data/                the DATA STAGE, hand-written Lua: data.lua, prototypes/,
                         ...
                         flag on 2.1 refuses the mod at load
...
test/                    headless verification, ELEVEN suites (see below), of
                         which two run on Factorio 2.1 -- `sedge`, the rule,
                         and `m1`, which is belt-free and needed nothing but a
                         manifest bump. The other nine build multi-edge rigs
                         and await the estate rebuild. And
...
make test     # headless verification. The DEFAULT is `m1 sedge`, which is
              # what runs on Factorio 2.1; SUITES=m2 names one of the nine
              # still built in the multi-edge idiom, all of which fail at the
              # loader ("Incompatible Factorio version") and would then fail
              # at the compiler. See test/run.sh's SUITES block.
              # (m1 m2 m3 upg plat mar edge mix mig qual sedge -- plat is the only
              ...
              # one-belt-per-part rule and the three ways of breaking it)
...
**The whole design is [`agents/single-edge.md`](agents/single-edge.md) and the whole implementation is `guest/go/sedge.go`; this is the working note that says where the seams are.** Phase 1 shipped 2026-08-24: the rule, its refusal, its merge arm and the `sedge` suite. The runtime-global setting, the grandfather pass, the fixture-based migration suite, the rebuilt test estate and the interactive worlds are later phases.
...
| **MAY the compiler use it?** a per-save policy | phase 2's runtime-global setting, and the seam for it is the return of `multiEdgeAllowed()` and nowhere else |
...
`m1` and `sedge`. The other nine suites are the multi-edge estate and their rebuild is a later phase; until then **nothing in this repo can measure the `mar` slopes**, so the "nothing on any hot path moves" claim above is structural rather than measured. That measurement is owed and belongs with the estate rebuild.
...
**THIS VERIFICATION IS NARROWER THAN EVERY ONE ABOVE IT AND THE GAP IS THE POINT.** The other nine suites cannot run -- 2.1 refuses their manifests, and their rigs are multi-edge -- so **the `mar` slopes were not measured and the `GC=leaking` arm was not run**, only built. Both are owed, and both belong with the test-estate rebuild rather than with this pass; the "nothing on any hot path moves" claims in "One belt per balancer part" are structural until then.

**What a future session should pick up first**, in order:

0. **The rest of the 2.1 port**, which is now the biggest thing outstanding and is chartered in [`agents/single-edge.md`](agents/single-edge.md): the runtime-global setting and its flip handler, the grandfather pass, the fixture-based migration suite over `test/fixtures-2.0/`, the nine suites' rigs rebuilt single-edge (which is what restores the `mar` slopes and the leaking arm as gates), the interactive worlds and the re-captured portal GIFs, and the `release/2.0` branch. Phase 1's own status section is the record of what is already done and of the two places its design needed correcting.
