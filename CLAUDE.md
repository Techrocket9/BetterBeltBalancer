...
- **0.3.3 IS BLOCKED FROM RELEASE, and this is a dated block that is removed when it lifts.** On Factorio 2.0.77 build 84539, a recipe-bound FkRecipes fallback note cannot be emitted at all: the library's own sentence is 247 bytes against a 200-byte-per-element ceiling on a localised string, so a typo a player types into `better-belt-balancer-recipe-ingredients` STOPS THE LOAD, and the `Error loading mods` dialog cannot reach the Mod Settings screen where it would be corrected. The unreleased 0.3.3 changelog and `README.md` both tell a player that such a text is set aside and the option above applies, and `README.md` says in as many words that the game loads; neither is rewritten, because they describe the release, the release is blocked, and the library fix is one shortened sentence. `test/check-datastage.py`'s `check_note_ceiling` holds the defect still in `note-refused`, with `note-control` beside it, and **A GREEN `note-refused` ROW IS THE BLOCK STILL IN FORCE.** It lifts on a measurement and not on a claim: the library's recipe sentences fitting 200 bytes with this mod's 39-byte setting name in them, `make datastage-check` green with `note-refused` inverted to the positive control, and the two documents re-read against a run that loads. The whole finding, its arithmetic, its grade against FkRecipes' own threat model and what the library owes are in [`agents/fkrecipes-migration.md`](agents/fkrecipes-migration.md), "Fix round 2", the fifth subsection.
...
**ONE THING OWED UPSTREAM.** When a chosen tier's copied pack and the declared `Fallback`'s one rung are the same name, `67b5090`'s new rung list names it twice: `... research takes at least one, and none of automation-science-pack, automation-science-pack is a science pack here`. Two tests here reach it and pin it as it is. It is cosmetic, it is in a sentence a player reads in an error dialog, and the fix is a de-duplication in the library's walk.
...
make datastage-check
              ...
              # suite can ask. TWENTY-ONE ARMS SINCE FIX ROUND 2'S
              # REMOVER COMMIT, two mod sets HASHED against goldens, sixteen
              ...
              # turned around. See Verification
