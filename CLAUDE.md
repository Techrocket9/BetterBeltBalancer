...
> **TRUNK TARGETS FACTORIO 2.1 SINCE 2026-08-24, AND ITS RULE IS ONE BELT PER BALANCER PART.** 2.1 closed the collision-mask loophole that let two interface linked belts share a part's tile, so a part carries at most one edge and a cluster that asks for more is refused. The design is [`agents/single-edge.md`](agents/single-edge.md) and the rule is `guest/go/sedge.go`; "One belt per balancer part" below is the working note. **Nine of the thirteen suites do not run on 2.1** -- every rig in them is built in the multi-edge idiom -- so every number in this file below that line was measured on 2.0.77 and is kept as measured until the estate is rebuilt. `m1`, `sedge`, `mig21` and `iact` are what `test/run.sh` runs today.
...
make test     # headless verification. The DEFAULT is `m1 sedge mig21 iact`,
              # which is what runs on Factorio 2.1; SUITES=m2 names one of the
              # nine still built in the multi-edge idiom, all of which fail at
              # the loader ("Incompatible Factorio version") and would then
              # fail at the compiler. See test/run.sh's SUITES block.
...
**FOUR OF THE THIRTEEN SUITES RUN ON FACTORIO 2.1, AND EVERY NUMBER IN THIS SECTION BELOW `sedge` WAS MEASURED ON 2.0.77.** The wall is in two layers, measured 2026-08-24 rather than assumed: 2.1 refuses a mod whose `info.json` says `factorio_version: 2.0` outright, so every suite fails at the loader before an entity is placed; and past that, every rig in nine of them puts two belts on one part, which is what 2.1 forbids. `m1` needed nothing but the one token and is green -- it is belt-free, so the rule cannot touch it -- `sedge` is the rule's own suite, `mig21` gets past both layers by not BUILDING a multi-edge world at all (it LOADS one out of `test/fixtures-2.0/`, which is the only way a 2.1 binary can ever be shown one), and `iact` is the interactive checklist's own staged world, rebuilt single-edge and gated with a single `--create`. Rebuilding the other nine single-edge is a later phase of the port ([`agents/single-edge.md`](agents/single-edge.md)'s test-estate table), and until it happens their numbers here are a record rather than a gate.

**Ten of the eleven suites run both phases under ONE mod set. `mig` is the exception**, and it has to be: what it tests is what happens when a NEIGHBOUR is installed or uninstalled, so its `BETWEEN` hooks rewrite `mod-list.json` AND add or delete a mod directory between the phases (a directory that is present but not listed is added back by Factorio as enabled, so "removed" has to mean both, and Factorio requires a mod directory to be named for the mod it holds, so a renamed stand-in has to be staged under a matching directory name). Its own section is "Adopting a Belt Balancer 2 or 3 save".
...
`m1`, `sedge`, `mig21` and `iact`. The other nine suites are the multi-edge estate and their rebuild is a later phase; until then **nothing in this repo can measure the `mar` slopes**, so the "nothing on any hot path moves" claims above are structural rather than measured. That measurement is owed and belongs with the estate rebuild.
...
0. **The rest of the 2.1 port**, which is now the biggest thing outstanding and is chartered in [`agents/single-edge.md`](agents/single-edge.md). Phases 1 and 2 are done -- the rule, its refusal, the setting, the grandfather pass, the migration and the `sedge` and `mig21` suites -- and what is left is **the nine suites' rigs rebuilt single-edge**, which is what restores the `mar` slopes and the leaking arm as gates and is the only thing that can pay off the "structural rather than measured" claims both phases carry; then the interactive worlds and the re-captured portal GIFs, and the `release/2.0` branch. **On that branch and nowhere else** live the 2.0-only legs phase 2 could not run: the grandfather write actually landing, both arms of the flip handler, and `sweepStackedInterfaces` against a standing multi-edge world. Each phase's own status section is the record of what is done and of where its design needed correcting.
