...
> **TRUNK TARGETS FACTORIO 2.1 SINCE 2026-08-24, AND ITS RULE IS ONE BELT PER BALANCER PART.** 2.1 closed the collision-mask loophole that let two interface linked belts share a part's tile, so a part carries at most one edge and a cluster that asks for more is refused. The design is [`agents/single-edge.md`](agents/single-edge.md) and the rule is `guest/go/sedge.go`; "One belt per balancer part" below is the working note. **Six of the thirteen suites do not run on 2.1** -- every rig in them is built in the multi-edge idiom -- so a number belonging to one of those six was measured on 2.0.77 and is kept as measured until the estate is rebuilt. `m1`, `sedge`, `mig21`, `m2`, `mar`, `upg` and `iact` are what `test/run.sh` runs today: `m2`, `mar` and `upg` had their rigs REBUILT single-edge on 2026-08-24 and their tables re-recorded on 2.1.14, which is also what restored the heap-slope gate the port's first three phases shipped without. `m3`, `plat`, `edge`, `mix`, `mig` and `qual` are the tranche that has not been rebuilt.
...
make test     # headless verification. The DEFAULT is `m1 sedge mig21 m2 mar
              # upg iact`, which is what runs on Factorio 2.1; SUITES=m3 names
              # one of the six still built in the multi-edge idiom, all of
              # which fail at the loader ("Incompatible Factorio version") and
              # would then fail at the compiler. See test/run.sh's SUITES
              # block.
...
**`m2`, `mar` and `upg` are the FIRST TRANCHE of the rebuilt estate, 2026-08-24.** Every rig in them was re-laid one belt per part -- which in practice means every column of parts became two, a west column carrying the row's inputs and an east column its outputs -- and every table below that describes what one of them asserts has been re-recorded on 2.1.14. The three that matter most are unmoved: `m2` delivers the same rates against the same control belt, the same item conservation across a recompile, and the same port count for every shape, because **the same balancer over twice as many parts is the same MACHINE**; `mar`'s per-operation heap slopes are unmoved on every term the 300-hour projection uses; and `upg` still adopts every network it finds rather than rebuilding one.

**`m3`, `plat`, `edge`, `mix`, `mig` and `qual` are the tranche that has NOT been rebuilt**, and every number in this section that belongs to one of them was measured on 2.0.77 and is a record rather than a gate. [`agents/single-edge.md`](agents/single-edge.md)'s test-estate table is the list.
