...
guest/go/obs/            THE TEST OBSERVERS, and the second half of "no
                         ...
                         (FKLUA-GAPS.md item 27). obs/protos and obs/obsdata are
...
test/                    headless verification, FOURTEEN suites (see below).
                         ...
                         guest/go/obs, and `m1`, `sedge`, `mar`, `mig21`,
                         `qual`, `mix`, `plat` and `mig` are gone from test/mods
                         since 2026-08-25
                         (agents/estate-port.md). Ten Lua files and 4,062
...
**AND SINCE 2026-08-25 THE TEST MODS ARE COMPILED GUESTS TOO, one phase at a time.** `m1`, `sedge`, `mar`, `mig21`, `qual`, `mix`, `plat` and `mig` are ported and their Lua is deleted; the other six, the interactive staging mod and the bench harness follow in [`agents/estate-port.md`](agents/estate-port.md)'s phases. Nothing about the sentence above moves -- an observer still asserts nothing -- and what the port makes load-bearing is the sentence AFTER it. **An observer's own log lines are a contract with the assertion script**: `test/assert-*.py` keys every regex on what follows the `[BBB-...]` tag and never on the `Script @__mod__/control.lua:N:` prefix the engine stamps in front of it, which is what lets a guest's `fk.Log` line satisfy a script written for a Lua mod's `log()` -- PROVIDED THE TEXT IS BYTE-IDENTICAL. So a phase is gated on a golden-log diff taken before a line is ported, and on a red proof that perturbing one token of one line fails the suite. The pilot's: **all 320 `[BBB]`, `[BBB-TEST]` and `[BBB-SEDGE]` lines across both suites and both phases byte-identical**, with only the elapsed-seconds column and the control.lua line number masked; and renaming `phase=` to `phaseno=` reports all nine of `m1`'s phases as never having run, while one space inserted into `sedge`'s `tick=` empties all three of its rate windows.
...
| **...and the TEST ESTATE is following the mod out of Lua** | An observer mod builds a world, drives it on a schedule and reports what it sees; fourteen of them were `control.lua` files. **Eight are compiled guests since 2026-08-25** -- `m1`, `sedge`, `mar`, `mig21`, `qual`, `mix`, `plat` and `mig` -- and their Lua is deleted, sharing one harness package and the mod's own generated bindings under `guest/go/obs`. The estate is 5,263 lines of Lua from 8,524. Every one of their log lines came back byte-identical against goldens taken before the port -- 320 in phase 1, 10,519 in phase 2, **1,290 across twenty logs in phase 3**, in order, with only a timestamp, a source attribution and a profiler's own measured milliseconds masked; a red proof per phase says the assertion surface is really exercised; and the mod's own heap slopes and member table have not moved by a byte through any of it. Eight phases; `agents/estate-port.md` |
...
**The standing programme is [`agents/estate-port.md`](agents/estate-port.md): the test estate out of Lua, one phase at a time.** Phases 1, 2 and 3 landed 2026-08-25 -- `m1` and `sedge` with the harness, the packaging recipe and the `copy_testmod` staging seam every later phase reuses; then `mar`, `mig21`'s observer and `qual`; then `mix`, `plat` and `mig`, which consumed the last piece of FkLua surface the port was waiting on and built the shared constants package. **Phase 4 is `m2`, `m3` and `edge`, the big ones**, and `flip` waits for a 2.0 binary. It is the biggest thing outstanding and it is the only one with a written charter, so start there unless something below is urgent. **Read its "gates a phase must clear" section before writing a line**, and in particular capture the goldens first: they are the one thing that cannot be recovered afterwards.
