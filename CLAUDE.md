...
**GATES.** `make check` **exit 0**; `cd guest/go && go test ./tune/ -count=1` **exit 0** over 52 tests; `../FkLua/bin/fklua gen-bindings --check` and `lock --check` **exit 0**; `GOTOOLCHAIN=go1.26.6 make mod` **exit 0**; `make datastage-check` **exit 0** after the re-capture, twenty arms. Every wasm build runs under `GOTOOLCHAIN=go1.26.6`, which is the re-adoption leg's environment fact unchanged: the host's Go is go1.27.1 and TinyGo 0.41.1 refuses it (`requires go version 1.19 through 1.26, got go1.27`). The package sizes, the relay's jump figures and the release-to-head proof are NOT TAKEN at this commit and belong to the round's close, when the settings dump has stopped moving.

**WHAT THE ROUND STILL OWES, AND THE FIRST OF IT IS THE ONE THING NO GATE ANYWHERE CAN SEE.** `--dump-data` does not read a locale file, the checker reads KEYS and never the strings beside them, and the one test that read a WORD of a description went with the value it was about, so **TEN ENTRIES IN `mod-data/locale/en/better-belt-balancer.cfg` GO ON INSTRUCTING THE PLAYER TO PICK AN OPTION THE DROPDOWN NO LONGER OFFERS with nothing red anywhere**: six descriptions and four names, listed in the migration note. That is the MISLED shape this round exists to close arriving through the round's own migration, and it is the next commit. After it: the two dropdown entries that are the 557 and the 622, the ten of eleven dropdown labels over the client's measured truncation, and the changelog sentence remedy (h) asks for, which is that a typo in the text field is now a way to change the recipe without touching the dropdown; then the remover fixture, remedy (i); then the round's close.
