...
make datastage-check
              ...
              # suite can ask. SEVENTEEN ARMS SINCE 0.3.3, ~40 s: two
              ...
              # and prerequisite, or a line of the library's own log, and one
...
**`make datastage-check` IS A SECOND GATE AND A DIFFERENT QUESTION, and it is not part of `make test`.** All fourteen suites are about the RUNTIME: they build a world, run it and read the guest's own log lines, and by the time any of them is looking the data stage has been over for a whole load. Not one of them can see a prototype FIELD. `test/check-datastage.py` runs Factorio's own `--dump-data`, which executes the settings and data stages and **stops before `control.lua`**, normalises the result with `jq -S` and hashes it against a golden in `test/datastage-goldens.json`. About three seconds per arm, against the suites' minutes. **SEVENTEEN ARMS SINCE 0.3.3 and about 40 seconds (`/usr/bin/time -p make datastage-check`: real 40.16 on 2026-09-08)**: the two hashed mod sets, fourteen VARIANT arms and one SPEED arm, which are not hashed and are described below; the six customizer arms are in "Round three" under "Cost, research and belt speed".
...
|---|---|---|--:|
| **2.0.77** `base` | `1e1fcf4f56f5ef22...` | `df4d6c3fc7bf854d...` | 3427049257 |
| **2.0.77** `incumbent` | `e7001bf98d6c6771...` | `df4d6c3fc7bf854d...` | 223071962 |
...
Captured, then RE-RUN against themselves to prove the dump is deterministic, and the whole gate was green on 2.0.77 -- eleven arms then, seventeen since 0.3.3: the two hashed arms, every variant arm and the speed arm, which builds its Go fixture mod and derives 0.5 from an underground exactly as it does on trunk. **The DECISION never waited for any of this**: `go test ./engine/` proves 2.0.x true, 2.1.x false and false-safe for anything unreadable, which is the same split `edgemode` already makes one stage later.
