...
**WHAT THE ROUND STILL OWES, AND THE ONE THING NO GATE ANYWHERE CAN SEE IS NOW HALF ANSWERED.** `--dump-data` does not read a locale file, the library's checker reads KEYS and never the strings beside them, and the one test that read a word of a description was deleted with the value it named, so ten entries of `mod-data/locale/en/better-belt-balancer.cfg` went on instructing the player to pick an option the dropdown no longer offers with nothing red anywhere. **THE SECOND COMMIT REWROTE ALL TEN**, cut the two dropdown entries from 557 and 622 characters to 443 and 563, and added the one test that can see the defect at all, which reads ONE CAPITALISED WORD PER ENTRY where the entries carry paragraphs. **STILL OWED, one commit each.** `README.md`'s customizer section is stale in at least five places, `:49`, `:63`, `:65`, `:67` and `:69`, and its "Left alone they are 20 automation science packs at 15 seconds each" is 0 and 0 with the tier deciding now; `mod-data/changelog.txt`'s unreleased 0.3.3 rows `:7`, `:8` and `:10` announce the option on both settings and need a REWRITE rather than remedy (h)'s one added sentence; the NINE dropdown labels, EIGHT of them at or past the client's measured ~37-character truncation (35, 38, 43, 45, 46, 50, 52, 58, 65, re-measured off the file), carry a census comment block with five sentences that count eleven labels and a value that is gone. Then the remover fixture, remedy (i); then the round's close.
