...
> **TRUNK TARGETS FACTORIO 2.1 SINCE 2026-08-24, AND ITS RULE IS ONE BELT PER BALANCER PART.** 2.1 closed the collision-mask loophole that let two interface linked belts share a part's tile, so a part carries at most one edge and a cluster that asks for more is refused. The design is [`agents/single-edge.md`](agents/single-edge.md) and the rule is `guest/go/sedge.go`; "One belt per balancer part" below is the working note. **Three of the thirteen suites do not run on 2.1** -- every rig in them is built in the multi-edge idiom -- so a number belonging to one of those three was measured on 2.0.77 and is kept as measured until the estate is rebuilt. `m1`, `sedge`, `mig21`, `m2`, `mar`, `upg`, `mix`, `plat`, `qual` and `iact` are what `test/run.sh` runs today: `m2`, `mar` and `upg` had their rigs REBUILT single-edge on 2026-08-24 -- which is also what restored the heap-slope gate the port's first three phases shipped without -- and `mix`, `plat` and `qual` followed the same day. `m3`, `edge` and `mig` are the tranche that has not been rebuilt.
...
mod-data/                the DATA STAGE, hand-written Lua: data.lua, prototypes/,
                         ...
                         prototypes/hidden.lua carries the OTHER conditional
                         prototype: `not_colliding_with_itself` and the
                         `bbb-can-stack` marker are emitted on 2.0.x and NEVER
                         on 2.1.x, from one branch on the data stage's `mods`
                         global, and the guard fails SAFE -- anything it cannot
                         read as 2.0.x is treated as 2.1, because emitting the
                         flag on 2.1 refuses the mod at load. settings.lua
                         defines `bbb-multi-edge-parts` behind the SAME branch,
                         and engine.lua is that branch: ONE FILE, required by
...
make test     # headless verification. The DEFAULT is `m1 sedge mig21 m2 mar
              # upg mix plat qual iact`, which is what runs on Factorio 2.1;
              # SUITES=m3 names one of the three still built in the multi-edge
              # idiom, all of which fail at the loader ("Incompatible Factorio
...
**One thing every rebuilt suite had to solve, and three of them arrived at it independently.** A conservation check forces a recompile by laying a belt on the cluster, and under this rule **a working balancer has no free face**: every part of it already carries its one belt, so that belt is now REFUSED and the check would measure a refusal instead of a teardown. Every such rig carries one extra EDGELESS part -- `m2`'s conservation rig, the interactive checklist's band B, and now every band of `mix` and `plat` -- and the belt goes there. Four independent passes needing the same trick is as strong a signal as this file has that it is the shape of the rule rather than a workaround.

**`m3`, `edge` and `mig` are the tranche that has NOT been rebuilt**, and every number in this section that belongs to one of them was measured on 2.0.77 and is a record rather than a gate. [`agents/single-edge.md`](agents/single-edge.md)'s test-estate table is the list.
...
The thirteenth suite, and **the only one that is not about this mod's behaviour at all**: it is about the world `test/interactive/bbb-interactive-setup` stages, which is what a human enables before walking the checklist and where the mod portal's demo scenes live. Nothing headless can make the gestures, which is the whole reason the checklist exists; but everything that mod STAGES is ordinary world-building, and a rig that stopped landing, or one this mod refuses, costs a human a whole session to discover and costs a single `--create` to catch.
...
**One `--create` and no benchmark**, because the whole question is answered by what the guest logged at load. **Red-proven** by staging a second belt against an occupied part: three assertions fire, one per family (a one-belt-per-part refusal, a compiled multiset missing that rig, and the second audit at `nets=11 refused=1`).
...
| question | channel |
|---|---|
| **CAN the engine stack?** a fact about the Factorio version | the data stage emits `not_colliding_with_itself` on 2.0.x and NEVER on 2.1.x, and defines the marker prototype `bbb-can-stack` in the SAME branch. The guest reads the marker, not a version -- so its belief cannot drift from the prototype's actual capability, because there is one `if` and not two. `mod-data/prototypes/hidden.lua` |
| **MAY the compiler use it?** a per-save policy | `settings.global["bbb-multi-edge-parts"]`, a **runtime-global** bool defaulting to false, which `mod-data/settings.lua` defines on 2.0.x and never on 2.1.x. Read behind the same cache; the seam is the return of `multiEdgeAllowed()` and nowhere else |
...
**The version guard fails SAFE.** Anything `hidden.lua` cannot read as 2.0.x is treated as 2.1, because emitting the flag on 2.1 refuses the mod at load while not emitting it on 2.0 merely costs the multi-edge geometry the guest would then refuse to build anyway.
...
- the bridge band's gap tile carries **one** flanking belt and not two, for that reason inverted: two would make the merge a single-edge refusal;
...
0. **The rest of the 2.1 port**, which is now the biggest thing outstanding and is chartered in [`agents/single-edge.md`](agents/single-edge.md). Phases 1, 2 and 3 are done -- the rule, its refusal, the setting, the grandfather pass, the migration, the `sedge` and `mig21` suites and the interactive and demo worlds -- and so is phase 4, the first tranche of the rebuilt estate: **`m2`, `mar` and `upg` are rebuilt single-edge and green in both arms**, which restored the `mar` slopes and the leaking arm as gates and paid off the "structural rather than measured" half of the earlier phases' claims. What is left is **`m3`, `plat`, `edge`, `mix`, `mig` and `qual` rebuilt the same way** -- `edge` and `mig` are the two that need real design work rather than a re-lay, because several of their rigs' gestures (a belt laid on a free face, the `lim` column's 64 belts over 32 parts, every adopted incumbent balancer) mean something different under the rule -- then the re-captured portal GIFs, and the `release/2.0` branch. **On that branch and nowhere else** live the 2.0-only legs phase 2 could not run: the grandfather write actually landing, both arms of the flip handler, and `sweepStackedInterfaces` against a standing multi-edge world. Each phase's own status section is the record of what is done and of where its design needed correcting.
