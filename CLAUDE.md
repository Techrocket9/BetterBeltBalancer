...
> **TRUNK TARGETS FACTORIO 2.1 SINCE 2026-08-24, AND ITS RULE IS ONE BELT PER BALANCER PART.** 2.1 closed the collision-mask loophole that let two interface linked belts share a part's tile, so a part carries at most one edge and a cluster that asks for more is refused. The design is [`agents/single-edge.md`](agents/single-edge.md) and the rule is `guest/go/sedge.go`; "One belt per balancer part" below is the working note. **ONE of the thirteen suites does not run on 2.1** -- every rig in `mig` is built in the multi-edge idiom -- so a number belonging to it was measured on 2.0.77 and is kept as measured until the estate is rebuilt. Everything else is what `test/run.sh` runs today: `m2`, `mar` and `upg` had their rigs REBUILT single-edge on 2026-08-24 -- which is also what restored the heap-slope gate the port's first three phases shipped without -- and `mix`, `plat`, `qual`, `m3` and `edge` followed the same day.
...
**`mig` is what has NOT been rebuilt**, and every number in this section that belongs to it was measured on 2.0.77 and is a record rather than a gate. [`agents/single-edge.md`](agents/single-edge.md)'s test-estate table is the list.
