...
**THE CUSTOM ARM BREAKS THIS RULE ON PURPOSE, AND THE STATE IT BREAKS IT IN IS THE UNTOUCHED ONE.** A written cost names no source, so its place comes from a ladder of its own that starts at `logistics-3`, chosen for the numbers a player who picked Custom is about to write rather than for the placeholder they start from. The placeholder is [FallbackUnit], base's `logistics` unit, so Custom with nothing edited is 20 red science sitting after Logistics 3: the mismatch this section argues against, with the sign flipped, for as long as the player has not typed. It is pinned as correct by `TestTheCustomResearchCostUntouchedIsTheFallbackUnit` and the gate's `tech-custom-default` arm, and recorded here so the next reader does not find it as a contradiction.
...
| injected | what fired |
|---|---|
| the `Position` ladder reduced to `logistics` | on the host, the two custom-cost tests, the ladder test and both arms of the unreadable-number test: `the prerequisites are ["logistics"] and the cost came from "logistics-3"`; at the gate, `tech-custom-default` and `tech-custom` on the prerequisite |
