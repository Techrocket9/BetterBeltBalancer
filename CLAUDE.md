...
test/                    headless verification, FOURTEEN suites (see below).
                         ...
                         (agents/estate-port.md). FOUR Lua files and 626 lines
                         left there, from twenty-one and 7,323, and not one of
                         them is a suite: two data-stage-only stand-ins and
                         `bbb-flip-test`, which cannot be ported on this
                         machine because its suite SKIPS on Factorio 2.1 and a
                         phase's first gate is a golden log.
...
| **...and the TEST ESTATE has followed the mod out of Lua** | An observer mod builds a world, drives it on a schedule and reports what it sees; fourteen of them were `control.lua` files. **EVERY SUITE THAT CAN BE PORTED IS A COMPILED GUEST since 2026-08-25** -- the pilot's `m1` and `sedge`, then `mar`, `mig21`, `qual`, then `mix`, `plat`, `mig`, and in phase 4 the three biggest, `m2`, `m3` and `edge` -- sharing one harness package and the mod's own generated bindings under `guest/go/obs`. The estate is **1,827 lines of Lua from 8,524, and none of what is left is a suite**. Every one of their log lines came back byte-identical against goldens taken before the port -- 320 in phase 1, 10,519 in phase 2, 1,290 in phase 3 and **4,220 in phase 4**, in order, with only a timestamp, a source attribution and a profiler's own measured milliseconds masked; a red proof per phase says the assertion surface is really exercised; and the mod's own heap slopes and member table have not moved by a byte through any of it. `flip` waits for a 2.0 binary, the interactive and bench mods are phases 5 and 7; `agents/estate-port.md` |
...
**The standing programme is [`agents/estate-port.md`](agents/estate-port.md): the test estate out of Lua, one phase at a time.** Phases 1 through 4 all landed 2026-08-25 -- `m1` and `sedge` with the harness, the packaging recipe and the `copy_testmod` staging seam every later phase reuses; then `mar`, `mig21`'s observer and `qual`; then `mix`, `plat` and `mig`, which consumed the last piece of FkLua surface the port was waiting on and built the shared constants package; and then `m2`, `m3` and `edge`, the three biggest, which is **every suite that can be ported on this machine**. What is left is **phase 5, the interactive staging mod** (which `iact` already gates headlessly, but whose README quotes tile coordinates the port moves), phase 6's two data-stage-only stand-ins, phase 7's bench harness -- which needs a comparability gate the suites do not, because every published performance figure in this repository was measured with those mods -- and phase 8's Rust parity exercise. `flip` waits for a 2.0 binary: its suite SKIPS on 2.1, and a phase's first gate is a golden log there is no run here to produce. **Read the "gates a phase must clear" section before writing a line**, and in particular capture the goldens first -- TWICE, which is what phase 4 found turns the mask list from an argument into a measurement -- because they are the one thing that cannot be recovered afterwards.
...
| [`agents/estate-port.md`](agents/estate-port.md) | **The test estate, in Go.** The programme for removing the last hand-written Lua in the repository: fourteen suites' observer mods, the interactive staging mod, two data-stage-only stand-ins and the bench harness, in eight phases. What an observer is and the two shipped-guest rules it does not inherit, the one-module/N-mains layout and the measurement that says an observer cannot bloat the mod, the packaging recipe (a neutral working directory, every identity a flag, `--api` not optional), the `copy_testmod` staging seam, the `[BBB-OBS] error:` gate that replaces Lua's `error()`, the six gates every phase must clear, the harness surface, and a record per phase -- the PILOT (`m1`, `sedge`), phase 2 (`mar`, `mig21`, `qual`) and phase 3 (`mix`, `plat`, `mig`), all 2026-08-25: every tagged log line byte-identical in order, a red proof each, the api-check verdicts that turned the stamped-vs-gated asymmetry into a number and produced the first `impacted` observer, and phase 3's verification of `fkapi.Log(Value)` against a golden profiler line before anything was built on it |
