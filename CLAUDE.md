...
**SINCE ROUND TWO THIS IS `fkrecipes`' `CostBy`, and it made one behaviour narrower.** The choice, the ladder walk, the verbatim unit copy and the prerequisite moving with it are all the library's, driven by `TechLadder` unchanged. What is new is that the library VALIDATES the fallback's science pack against the game before it resolves anything, so a pack with a perfectly good `logistics` and no `automation-science-pack` in it is refused at plan time where the hand-rolled version loaded. It is a narrower failure than the one it replaces rather than a new class -- a fallback that DID fire in such a pack emitted a unit naming a missing item, which is the engine's own assignID abort -- and `TestTheFallbackPackIsProbedEvenWhenUnreachable` pins the sentence so the day the library probes lazily is a day this repo is told about.
...
**ONE BEHAVIOUR IS NARROWER AND IT IS ON THE RECORD RATHER THAN DISCOVERED.** The library validates `CostChoices.Fallback` before it resolves anything, so the fallback's science pack is presence probed whether or not the fallback is reachable: a pack with a working `logistics` and no `automation-science-pack` is now refused at plan time (`fkrecipes: the technology bbb-balancer prices itself in automation-science-pack, which does not exist`) where the hand-rolled version loaded. There is no way to declare a `CostBy` without a fallback, the failure it replaces is narrower rather than absent, and a test pins the sentence.
