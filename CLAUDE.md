...
**And RELEASED-ALIGNED 2026-09-14: FkRecipes arrives through the real channel.** `guest/go/go.mod`'s dev-only replace onto the sibling checkout is gone; the require of `github.com/Techrocket9/fkrecipes/go v0.1.0` resolves the library's `go/v0.1.0` tag on GitHub and `guest/go/go.sum` carries its two lines. The repository is PRIVATE until a build relying on it has shipped, so the Makefile exports `GOPRIVATE` for that path and a developer maps GitHub to SSH in their own git config for the fetch (the Makefile's comment has both forms); nothing here writes that config, and a machine that has fetched once builds offline from the module cache. FkLua stays a sibling replace, as its go.mod comment explains. At this head: `fklua gen-bindings --check` 0, `fklua lock --check` 0 (api 2.1.17), `make check` 0, `make mod` 0, `make datastage-check` 0 on both mod sets with no golden moved, `make zip` 0 (`better-belt-balancer_0.3.3.zip`, 902,925 B). The 0.3.3 changelog is dated for the release.
