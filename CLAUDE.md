...
| **...and the TEST ESTATE is following the mod out of Lua** | An observer mod builds a world, drives it on a schedule and reports what it sees; fourteen of them were `control.lua` files. `m1` and `sedge` are compiled guests since 2026-08-25 and their Lua is deleted, sharing one harness package and the mod's own generated bindings under `guest/go/obs`. All 320 of their log lines came back byte-identical against goldens taken before the port, both suites, both phases, with only a timestamp and a line number masked; two red proofs say the assertion surface is really exercised; and the mod's own heap slopes and member table did not move by a byte. Eight phases; `agents/estate-port.md` |
...
**The standing programme is [`agents/estate-port.md`](agents/estate-port.md): the test estate out of Lua, one phase at a time.** Phase 1 -- `m1` and `sedge` -- landed 2026-08-25 with the harness, the packaging recipe and the `copy_testmod` staging seam that every later phase reuses; phase 2 is `mar`, `mig21`'s observer, `qual` and `flip`. It is the biggest thing outstanding and it is the only one with a written charter, so start there unless something below is urgent. **Read its "gates a phase must clear" section before writing a line**, and in particular capture the goldens first: they are the one thing that cannot be recovered afterwards.
