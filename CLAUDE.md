...
> **TRUNK TARGETS FACTORIO 2.1 SINCE 2026-08-24, AND ITS RULE IS ONE BELT PER BALANCER PART.** 2.1 closed the collision-mask loophole that let two interface linked belts share a part's tile, so a part carries at most one edge and a cluster that asks for more is refused. The design is [`agents/single-edge.md`](agents/single-edge.md) and the rule is `guest/go/sedge.go`; "One belt per balancer part" below is the working note. **Nine of the twelve suites do not run on 2.1** -- every rig in them is built in the multi-edge idiom -- so every number in this file below that line was measured on 2.0.77 and is kept as measured until the estate is rebuilt. `m1`, `sedge` and `mig21` are what `test/run.sh` runs today.
...
test/                    headless verification, TWELVE suites (see below), of
                         which three run on Factorio 2.1 -- `sedge`, the rule;
                         `m1`, which is belt-free and needed nothing but a
                         manifest bump; and `mig21`, which does not BUILD a
                         multi-edge world but LOADS one out of
                         fixtures-2.0/, which is the only way a 2.1 binary can
                         ever be shown one. The other nine build multi-edge
...
make test     # headless verification. The DEFAULT is `m1 sedge mig21`, which
              # is what runs on Factorio 2.1; SUITES=m2 names one of the nine
              ...
              # (m1 m2 m3 upg plat mar edge mix mig qual sedge mig21 -- plat is
              ...
              # committed fixture IS phase one)
...
**THREE OF THE TWELVE SUITES RUN ON FACTORIO 2.1, AND EVERY NUMBER IN THIS SECTION BELOW `sedge` WAS MEASURED ON 2.0.77.** The wall is in two layers, measured 2026-08-24 rather than assumed: 2.1 refuses a mod whose `info.json` says `factorio_version: 2.0` outright, so every suite fails at the loader before an entity is placed; and past that, every rig in nine of them puts two belts on one part, which is what 2.1 forbids. `m1` needed nothing but the one token and is green -- it is belt-free, so the rule cannot touch it -- `sedge` is the rule's own suite, and `mig21` gets past both layers by not BUILDING a multi-edge world at all: it LOADS one out of `test/fixtures-2.0/`, which is the only way a 2.1 binary can ever be shown one. Rebuilding the other nine single-edge is a later phase of the port ([`agents/single-edge.md`](agents/single-edge.md)'s test-estate table), and until it happens their numbers here are a record rather than a gate.
...
**The whole design is [`agents/single-edge.md`](agents/single-edge.md) and the whole implementation is `guest/go/sedge.go`; this is the working note that says where the seams are.** Phases 1 and 2 shipped 2026-08-24: the rule, its refusal and its merge arm, then the runtime-global setting, the grandfather pass, the migration and the `sedge` and `mig21` suites. The rebuilt test estate and the interactive worlds are the remaining phase.
...
`m1`, `sedge` and `mig21`. The other nine suites are the multi-edge estate and their rebuild is a later phase; until then **nothing in this repo can measure the `mar` slopes**, so the "nothing on any hot path moves" claims above are structural rather than measured. That measurement is owed and belongs with the estate rebuild.
