...
**AND THE REFUSAL'S OWN ALERT SAYS SOMETHING FALSE ON THIS PATH, WHICH IS FOUND AND NOT FIXED HERE.** `guest/go/sedge.go`'s line ends *"so the compile is refused BEFORE the teardown and the standing network is untouched"*, and both clauses are true when a BELT edit provokes the refusal and false when a PART REMOVAL does — which is what this gesture is. Measured verbatim in the `frepd` window, six items on the floor two lines below it. It is the same shape as the false sentence [`agents/single-edge.md`](agents/single-edge.md)'s phase 8 records for a converted balancer, and it is one sentence shared by two bounds and read by four suites' regexes, so it is written down here rather than changed in passing.
