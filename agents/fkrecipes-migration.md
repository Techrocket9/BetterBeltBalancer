...
**AND THE FALLBACK'S BOUNDARY IS PINNED RATHER THAN LEFT AS A SENTENCE.** None of the seven texts above is a case the fallback cannot carry, because that fixture has every rung of every ladder. The case it cannot is the same `1 water` in a game whose only science pack is `logistic-science-pack`, where this mod's declared list has nothing to land on either. **AS MEASURED IN FIX ROUND 1** the load stopped there, with `fkrecipes: the technology bbb-balancer has no science pack the game has; research takes at least one. The stored value of better-belt-balancer-tech-packs could not be used, so the mod's own declaration applied; correcting it under Settings > Mod settings > Startup is what a player can change here.`, whose second sentence was the library's own and was the whole difference between that refusal and the one a player who typed nothing got in the same game. **THAT IS NOT WHAT HAPPENS SINCE FkRecipes `97a4493`, 2026-09-14, AND THE CORRECTION IS HERE RATHER THAN IN A REWRITTEN PARAGRAPH**: the load COMPLETES, the research is emitted with no science pack and completes for free, the technology's own description carries one line saying this game has none of the science packs it names, and what became of the player's text is the LAST of five `fkrecipes:` log lines rather than a clause of a refusal. The test that pins it is `TestAFallbackThisModsOwnPackListCannotPayForIsEmittedFreeBesideTheSetAsideText`, and the boundary where a refusal still stands after a fallback, carrying the library's added sentence, is `TestARefusalRaisedAfterAFallbackSaysWhatTheStoredValueDid`. See "Fix round 3".
...
## Fix round 3, 2026-09-14: the library stops refusing an environment, and this mod adopts

**THIS SECTION IS WRITTEN AS THE ROUND LANDS AND ITS CLOSE IS OWED.** Two more commits follow in this round and will append their own subsections; what is here is the leg that adopts the library and the two documents that leg falsified. The red proofs, the sizes, the relay row, every gate at its exit code and what the round closes are the later commits' to earn, and nothing below stands in for them.

FkRecipes fix round 3 answers this repository's THIRD migration assessment, `agents/migration-assessment-3.md` taken at `313f9d3`, whose two BLOCKED findings were one outcome between them: the load stopping on a mod set the player did not assemble, from an `Error loading mods` dialog with no route to the Mod Settings screen and no way past it but Reset mod settings, which discards every startup preference in the file. **THE HEADLINE IS A PLAYER-VISIBLE OUTCOME AND IT IS NOT AN UNMIXED GOOD.** Where a modpack leaves this mod's research with no science pack the game has, the load used to stop; it completes now and the research is emitted with an empty cost, which means a research this mod meant its players to pay for costs them nothing. That is a balance change nobody chose, disclosed in one log line and one line of the technology's own description rather than prevented. What would prevent it here is a second rung under this mod's declared pack, which `guest/go/tune/plan.go` still declines and which this leg did not write.

### What moved in the library, in this mod's terms

FkRecipes `61ac80c` to `cf66357`, six commits. Five of them move a byte this mod emits, reads or documents, and each row's attribution was taken with `git -C ../FkRecipes log -S'<literal>' --oneline -- go rust` over a literal from the change itself rather than from the commit subject.

| commit | what it is | what it does here |
|---|---|---|
| `97a4493` | the packless refusal is DELETED. A technology priced in no science pack the game has is emitted with an empty `ingredients` list, one `fkrecipes: ERROR: ` line per technology naming every rung the walk asked about, and one line in that technology's own description | the outcome in a demoted-pack modpack changes from a stopped load to a FREE research. Three tests here went red by name and are exit-0 tests now; `README.md:70`, four present-tense lines of `CLAUDE.md` and one present-tense paragraph of this file described the refusal as the outcome, and five dated records here name the tests it was pinned by |
| `ca4c344` | every sentence about a stored value that could not be used says the field behaved as though it had been LEFT ALONE, where they said this mod's own default, choice, list or declaration applied. Four of them reach this mod: the note on the recipe or the technology, the `fkrecipes: ERROR: ` line, the closing line of each text setting's composed description, and the sentence a refusal adds after a fallback | this repository's finding 19 answered. The fallback target beside a dropdown is the preset the player is standing on, which the assessment measured false on five of this mod's six presets. Two tests red; the composed setting descriptions move (the closing fallback line 147 bytes to 162) and the recipe's own note grows 247 bytes to 268 |
| `95b7a2c` | the degradation that leaves a research with no prerequisite and no copied cost records a note where a player looks | this mod REACHES it and nothing here pins it, which is read off the library's own call site (`go/data.go`, the arm where no source in the chosen ladder carries a unit) rather than off a run taken here: in a game with no logistics technology at all, which is `TestNoLogisticsAtAllIsTheFallbackAndNoPrerequisite`'s world, the emitted technology now carries `No technology this research takes its cost from carries a cost this mod can use here, so this research has no prerequisite and no copied cost. The reason is in the log.` That test asserts the unit, the absent prerequisite and the log line and says nothing about the description, which is why it stayed green |
| `121aafc` | a note JOINS the author's own locale description instead of replacing it: the composition opens with the engine's own `[<kind>-description]` key for the prototype being emitted, in the alternatives form with the separator newline inside the alternative | this repository's finding 21 closed with NOTHING for this side to do. This mod's technology description lives in a locale file, `[technology-description] bbb-balancer`, and a prototype's own `localised_description` field wins over the locale entry of the same name, so the sentence saying what the research IS was REPLACED by the disclosure the moment anything fell back; it is composed above the note now. The RECIPE channel had nothing to lose and still has not, which is the asymmetry rather than a gap: this mod ships no `[recipe-description]` section at all (the `bbb-balancer-part` description it does write is an `[item-description]`, a different prototype), so the key the library composes there resolves to nothing and the note stands alone exactly as before. The library composes it on both because it cannot read a .cfg. Neither description moves into a plan field and the library does not ask |
| `933d4d3` | the ladder disclosure is a sentence the library composes, in three vocabularies: one for an ingredient dropdown, one for an ingredient text field and one for a pack text field | this repository's finding 22 answered, which was that the library's design cited a disclosure THIS MOD'S locale file was the only copy of. One test red; three of this mod's four composed setting descriptions gain a line; and the copy in `mod-data/locale/en/better-belt-balancer.cfg` is deleted in this leg |

**`cf66357` IS THE HEAD THIS LEG IS MEASURED AGAINST AND IT IS NOT IN THE TABLE.** It measures finding 15's cosmetic residual, the `Logistics 2: cost of logistics-2` line, and DECLINES it: the settings stage sees no `data.raw` at all, a setting prototype is neither readable nor writable from the data stage, `mod-settings-dump.json` is already on disk before the data stage runs, and `logistics-2` carries no `localised_name` in the dump to copy (3 of 275 technologies do). Its code change is two doc comments in the library and one paragraph of `docs/usage.md`; nothing this mod emits or reads moves for it.

### The baseline, the untouched tree at `313f9d3` against library `cf66357`

**THE PLAN COMPILES UNCHANGED, WHICH IS THE DIFFERENCE FROM FIX ROUND 2.** That round's baseline had four declarations in `tune.Plan()` that stopped compiling. Here nothing does: `git diff -U0 -- guest/go/tune/plan.go | grep '^[+-]' | grep -v '^[+-][+-]' | grep -vE '^[+-]\s*//'` prints NOTHING at this leg's HEAD, so nothing executable in the guest changed, and the whole adoption cost is six test expectations.

**THE UNTOUCHED RUN WAS TAKEN WITHOUT TOUCHING THE TREE OR EITHER SIBLING CHECKOUT.** `git archive HEAD` out of this repository unpacked into a scratch directory sitting beside symlinks to the two checkouts, so that `guest/go/go.mod`'s two `../../../` replaces resolve onto the real FkLua and the real FkRecipes at `cf66357`, and `cd guest/go && go test ./tune/ -count=1` run there. Exit **1**, six tests. Nothing was written to this repository, to FkLua or to FkRecipes to get it.

Factorio **2.0.77 (build 84539, mac-arm64, steam)**, the binary re-asked its version before every engine row. `go test` needs no engine and no wasm toolchain, which is why the baseline above is the cheap half.

### What went red on the untouched tree, which is the whole of what this leg adopted

Six tests, all in `guest/go/tune`, every one of them pinning a library sentence or a library shape word for word, and every one red for the reason it was pinned. The texts below are what the run above printed.

**THE THREE REFUSAL TESTS, WHICH ARE EXIT-0 TESTS NOW.** Each failed on its own "the library planned cleanly where this pinned a refusal" message rather than on a diff, because the thing they asserted was an error and there is no error any more.

- **`TestAReachedFallbackWithNoPackInTheGameIsRefused`**: `a game with no science pack priced this mod's research anyway: a research that costs nothing is one a player finishes by opening the screen`. It is `TestAReachedFallbackWithNoPackInTheGameIsEmittedFreeAndDisclosed` now, and it asserts the emitted unit, the absent prerequisite, the packless note on the technology's own description and the whole three-line log stream.
- **`TestAFallbackThisModsOwnPackListCannotPayForStillRefuses`**: `a game with no automation science pack in it priced this mod's declared research anyway: the fallback landed on a pack nobody defined, which is a load error with this mod's name on it`. It is `TestAFallbackThisModsOwnPackListCannotPayForIsEmittedFreeBesideTheSetAsideText` now, five log lines with the player's own last.
- **`TestAPackTheGameHasOnlyAsAnItemIsDroppedAndThenRefused`**: `a game holding this mod's science pack as an ordinary item priced its research anyway: the pack ladder is asking ItemExists, and the engine takes tool-type items in a unit and nothing else`. It is `TestAPackTheGameHasOnlyAsAnItemIsDroppedAndTheResearchIsEmittedFree` now. **THAT IS THE ASSESSMENT'S OWN FIXTURE SHAPE**: a pack still in the game but no longer a `tool`, which is what a modpack does when it converts science, and the mod set findings 13 and 14 were measured on.

**THE TWO NOTE TESTS, EACH RED TWICE OVER.** `TestATextTheGameCannotAnswerFallsBackAndSaysSo` (four rows) and `TestAPackTextTheGameCannotAnswerFallsBackAndSaysSo` (four rows) share `checkFallbackNote`, and both halves of what it asserts moved.

- The SHAPE, once per row: `localised_description element 1 is ["?", ["", ["recipe-description.bbb-balancer-part"], "\n"], ""], and every parameter of a composed note is a string`, and the same with `technology-description.bbb-balancer` on the pack side. That is `121aafc` arriving, and the helper's own message is what named it: the element it called a stray table is the engine's own description key for that prototype, composed above the note.
- The SENTENCE, once per row: `The mod loaded as though that text had been left alone; fix the text under Settings > Mod settings > Startup, then restart.` against a `want` reading `The mod loaded with its own default instead; ...`. That is `ca4c344` arriving, on all eight rows across the two tests, and the route at the end of the line is unchanged because a LOG line is read by a player who is in the game.

**AND ONE SETTINGS TEST, WHICH IS THE ONLY ONE THAT SAW `933d4d3` AT ALL.** `TestEverySettingPrototypeIsTheOneThatShipped` transcribes all six setting prototypes field by field, and three of the four composed descriptions came back one parameter longer than its literal: `bbb-recipe-cost` with `\nWhere an option names something your mods do not have, ...` between its wrap line and its switch line, and both text fields with the same rule in their own vocabulary between their wrap line and their format line. The fourth, `bbb-tech-cost`, is unmoved, and that is the library's rule rather than a gap: a cost preset renders a localised label and a localised technology name, so there is no typeable list for either the wrap line or the ladder line to be about. The two text fields' closing fallback line moved in the same test, which is `ca4c344` again.

**AT THIS LEG'S HEAD `cd guest/go && go test ./tune/ -count=1` IS EXIT 0 OVER 54 TESTS**, 53 at the previous round's close plus `TestARefusalRaisedAfterAFallbackSaysWhatTheStoredValueDid`, which pins the one sentence the library adds on behalf of a player whose typed value was set aside on the way to a refusal and which nothing in this repository pinned before.

### The goldens, and what moved in them

**THE SETTINGS HASH MOVED ON BOTH MOD SETS AND THE DATA HASH MOVED ON NEITHER.** Factorio 2.0.77, both hashed arms, read out of `test/datastage-goldens.json` and the run that re-captured it:

| | base | incumbent |
|---|---|---|
| `mod_settings_sha256` | `b6e583e6163249ba` to **`f83e037d8f787dcf`** | the same move, the same two values |
| `data_raw_sha256` | **`1e1fcf4f56f5ef22`**, unmoved | **`e7001bf98d6c6771`**, unmoved |
| `prototype_list_checksum` | **3427049257**, unmoved | **223071962**, unmoved |

The settings move is three settings and nowhere else, every part of it read out of the engine's own dump: `bbb-recipe-cost` and both text fields gained the ladder line, and both text fields' closing fallback line was reworded. `bbb-tech-cost` and the two research numbers are untouched.

**BEFORE THE RE-CAPTURE `make datastage-check` WAS EXIT 2 WITH EXACTLY TWO FAILURES, BOTH `mod_settings_sha256`, ONE PER MOD SET**, which is the shape that says the change reached the settings stage and nothing else; after it, exit **0**. The golden's own `_note` carries the same account, re-recorded by the `--update` path that captured it.

**THAT THE DATA HASH DID NOT MOVE IS ITSELF THE CHECK THE LIBRARY ASKED FOR, AND IT IS WORTH MORE THIS ROUND THAN LAST.** Fix round 3 changed what a prototype carrying a NOTE emits and what a technology that loses its packs emits, so a data hash moving here would have meant an ingredient list or a research unit resolving differently on the DECLARED DEFAULTS, in a mod set where nothing is missing and nothing was typed. That would be a defect, and finding it before it ships is what the row is for. The configurations that DO degrade are not hashed at all, by design: they are the gate's own note arms and the host suite's fixtures.

**THE TWO NOTE ARMS, OUT OF A REAL 2.0.77 DUMP AT THIS LEG'S HEAD.** `note-recipe` carries the 268-byte recipe note in two chunks of 179 and 89 bytes under a composed `recipe-description.bbb-balancer-part` key, which this mod does not define and which therefore resolves to nothing; `note-control` carries the 159-byte technology note in ONE chunk under `technology-description.bbb-balancer`, which this mod DOES define and which is the sentence finding 21 was measured losing. Both numbers moved with the sentence rather than with the chunker: the recipe note was 247 bytes cut 176 + 71 at the previous round's close, and 229 of the 268 are library constant with this mod's 39-byte setting name making up the rest. Both arms assert the composed key at slot 1 now, one with an entry behind it and one without, which is finding 21 asserted on an engine rather than in a host suite alone.

### The two documents this round falsified, and what they say now

**`README.md:70` TOLD A PLAYER THE LOAD WOULD STOP.** It read, in part: "The one thing that can still stop the load is a research with no science pack at all, and there are two ways into it. ... If your game has no automation science pack either, the research would have no packs at all, and a research with none is free rather than cheap, so the load stops instead with a message saying so." The one clause of that still standing is that a research with no packs is free rather than cheap; what is false is everything the paragraph did with that fact, from "the one thing that can still stop the load" to "the load stops instead", and the paragraph's opening claim that the presets and the tiers "are safe in an overhaul pack" with it. What it says instead is that the game loads, that the research is built with an empty cost and completes for free, that its own description in the technology screen says this game has none of the science packs it names, and that a research you were meant to pay for costing nothing is a balance change you did not choose, with the science packs field named as the thing a player can do about it. **TWO SMALLER CORRECTIONS WENT WITH IT, BOTH FROM THE ASSESSMENT'S OWN REMEDY LIST.** The paragraph said "missing from your game" where the measured case is a DEMOTION, a science pack still in the game but no longer a `tool` prototype, so it says "missing from your game, or no longer a science pack in it" in both places a pack is asked about; and the opening claim reads "built to survive an overhaul pack, and the one place they can run out is the science packs", because the paragraph's own last sentences are where they run out.

**`mod-data/locale/en/better-belt-balancer.cfg:203` WAS THE ONLY COPY OF THE LIBRARY'S OWN RULE, AND THE COPY IS DELETED.** The `bbb-recipe-cost` entry carried, verbatim, "where one names something your mods do not have, the nearest thing they do have is used instead, and where that leaves one item named twice the two amounts are added, so what you craft can be a shorter list than the one shown". FkRecipes `933d4d3` composes that rule onto the same tooltip, between the wrap line and the switch line, in its own words. THREE REASONS, recorded in the file's own comment block: this file's stated policy forbids repeating what the library composes, because a player who only ever picks a preset would read it twice in one tooltip; the library's sentence is maintained beside the walk that resolves the ladders and this one was maintained here, which is how two copies drift; and finding 22 was precisely that the library's design cited a disclosure only this consumer composed, which left every other consumer free to write it differently or not at all. Keeping a second copy re-opens the seam it was closed on.

**WHAT THE ENTRY KEEPS IS WHAT THE LIBRARY DOES NOT SAY**, and the contrast was checked against the engine's dump rather than against the library's source: what a balancer part costs to craft, that the default is what this mod has always used, and that a list you write yourself is taken as written with nothing substituted for a name you typed. Over the settings dump this gate leaves at `test/tmp/datastage/base-settings.json`, which is untracked scratch output of a 2.0.77 run of this tree rather than a committed fixture, the strings `as written`, `nothing is substituted`, `you type` and `you write` appear ZERO times, so the last of the three is this mod's alone. The library's own ladder line is the near miss and is not the same claim: it says where a list THIS MOD CHOSE names something absent the next name is used, which is silent about a list the player typed.

**NO GATE ANYWHERE CAN SEE THAT DELETION, AND THE COMMENT BLOCK SAYS SO.** `--dump-data` holds the locale KEY and never the string, so no hash in `test/datastage-goldens.json` can move for an edit to this file, and FkRecipes' own locale checker reads keys too. `TestNoSettingDescriptionNamesAnOptionNoDropdownOffers` in `guest/go/tune/locale_test.go` is the whole of what any suite reads here, and it reads capitalised words against the option lists this plan declares. The entry is still non-blank, so the REQUIRED check is satisfied either way.

**AND `CLAUDE.md` CARRIED FOUR PRESENT-TENSE ASSERTIONS OF THE DELETED REFUSAL AND ONE STALE BYTE COUNT.** Its Status table row said a mod set that cannot pay for what a fallback lands on stops the load and that the typed list falls back to this mod's own declared list; both are corrected in place. Three dated round records (`Round three`, `Fix round 1` and the fix round 2b red-proof list) keep their dates and gain an annotation naming `97a4493`, `ca4c344` and the three test renames, which is this file's own habit for a record whose subject moved. And the critical rule about the 200-byte element ceiling carried `176 + 71` in the present tense; the cut is `179 + 89` and the composition is four elements rather than three, both moved by this round and both measured by the note arms above.
