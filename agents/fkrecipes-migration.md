...
**STILL OPEN, UNCHANGED BY THIS LEG**: the client run and its two hovers; the ten of eleven dropdown labels over the client's measured truncation; `bbb-tech-cost` carrying no overhaul-pack sentence where its twin does; the `Sync mods with save` route being unpromised; "a cost gates softly where a prerequisite gates hard" being reasoning rather than a probe; and the 2.0 branch's own 0.2.3 changelog entry, which stands until the next recut.
