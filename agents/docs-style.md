...
**Read before creating or editing any document a reader outside this project will see**: `README.md`, `FKLUA-GAPS.md`, `bench/README.md`, `bench/baselines/*.md`, `test/interactive/README.md`, and any future `docs/`. It does not apply to `CLAUDE.md`, `agents/*.md`, `.claude/`, prompt files, code comments, or generated files; those are working notes and keep their own voice.
...
- **Local absolute paths** (`/Users/...`, `~/Library/...` presented as the reader's). Describe generically ("a local FkLua checkout", "your Factorio user directory") or link to GitHub. Sibling-checkout links like `../FkLua` become GitHub URLs (the one exception is the build requirement itself, which really is a sibling checkout at `../FkLua` and may be stated as such): `https://github.com/Techrocket9/<Repo>` for a repo, `https://github.com/Techrocket9/<Repo>/blob/master/<path>` for a file.
...
BetterBeltBalancer is released under the MIT License (`LICENSE` at the repository root), and so is FkLua, which compiles it. Say so plainly, link the file, and do not imply anything else anywhere.
