// Command bbb-edge-test-data is the `bbb-edge-test` observer's DATA STAGE: a
...
// thing every other suite's kit has, for the same reason. See obs/obsdata, and
// obs/protos for where the NAME lives so that this module and obs/edge can both
// have it without sharing an import.
//
...
//go:noinline
func onData() { obsdata.ExpressLoader(protos.EdgeLoader) }
