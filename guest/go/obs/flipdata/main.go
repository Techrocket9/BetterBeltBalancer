// Command bbb-flip-test-data is the `bbb-flip-test` observer's DATA STAGE: a
...
// rigs is a stock express belt or chest, so what the suite measures is the rate a
// real one delivers.
//
...
// which is what lets both this module and obs/flip have them. The five calls that
// clone the loader live in obs/obsdata, which every data stage in the estate
// shares. See agents/estate-port.md, phase 9.
package main
...
//go:noinline
func onData() { obsdata.ExpressLoader(protos.FlipLoader) }
